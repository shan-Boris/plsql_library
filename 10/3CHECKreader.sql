-- Создай метод по проверке может ли читатель взять книгу (кейсы: плохой рейтинг у читателя, читатель уже взял эту книгу, книга не подходит по возрасту и т.д.)
        ...
        CASE WHEN ((SELECT
                ...
                AND (SELECT 
                    ...
                    WHERE DRB.RETURN_BOOK IS NULL
                        AND C.ID = ID_CLIENT$
                        AND B.TITLE = TITLE$) = 0 -- если еще не взял эту книгу
                AND (SELECT
                        RATING
                    FROM CLIENT C
